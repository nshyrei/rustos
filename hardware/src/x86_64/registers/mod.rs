...
pub fn cs() -> u16 {
    ...
    unsafe { llvm_asm!("mov %cs, $0" : "=r" (result)) }

    result
}

#[inline(always)]
pub unsafe fn cs_write(val : u64) {
    asm!(
            "push {sel}",
            "lea {tmp}, [1f + rip]",
            "push {tmp}",
...
#[inline(always)]
pub unsafe fn rflags_write(val : u64) { llvm_asm!("pushq $0; popfq" :: "r"(val) : "memory" "flags") }
...
#[inline(always)]
pub unsafe fn sp_write(val : u32) { llvm_asm!("mov $0, %esp" :: "r" (val) : "memory") }
...
pub fn sp_read() -> u32 {
    ...
    unsafe { llvm_asm!("mov %esp, $0" : "=r" (result)) }
